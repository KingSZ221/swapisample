...
namespace wpfapp.bu.sketch.action.compose.cube
{
    /// <summary>
    /// 创建立方体
    /// </summary>
    public class CreateCubeAction : SwSketchActionBase
    {
        ...
        public CreateCubeAction(object oInVo) : base(oInVo)
        {
        ...
        private RespVo step1(CreateCubeInVo oInVo, CreateCubeOutVo oOutVo)
        {
            ...
            oRespVo = SwBuSketchService.getInstance().executeSketchAction(EnumSwSketchActionType.EditSketch, oOutVo.SketchEditInVo);
            if (!oRespVo.ok)
            {
                return oRespVo;
            }
            ...
            // 绘制图形
            oRespVo = SwBuSketchService.getInstance().executeSketchAction(EnumSwSketchActionType.CreateCenterRectangle, oOutVo.ContourInVo);
            if (!oRespVo.ok)
            {
                return oRespVo;
            }
            ...
            oRespVo = SwBuSketchService.getInstance().executeSketchAction(EnumSwSketchActionType.ExitSketch, oOutVo.SketchExitInVo);
            if (!oRespVo.ok)
            {
                return oRespVo;
            }
        ...
        private RespVo step2(CreateCubeInVo oInVo, CreateCubeOutVo oOutVo)
        {
            ...
            // 绘制图形
            oRespVo = SwBuSketchService.getInstance().executeSketchAction(EnumSwSketchActionType.FeatureExtrusion, oOutVo.FeatureCube);
            if (!oRespVo.ok)
            {
                return oRespVo;
            }
